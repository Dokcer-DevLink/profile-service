...
//    ProfileDto findProfileDtoById(Long id);
    ...
    @Query("select new com.goorm.devlink.profileservice.dto.ProfileDto(p.id, p.imageUrl, p.name, p.nickname, p.introduction, p.stacks, p.address) from ProfileEntity p where p.id = :id")
    ...
    @Query("select new com.goorm.devlink.profileservice.dto.ProfileDto(p.id, p.profileUuid, p.imageUrl, p.nickname, p.stacks, p.address) " +

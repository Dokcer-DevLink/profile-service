...
public interface ProfileRepository extends JpaRepository<ProfileEntity, Long> {//, ProfileRepositoryCustom {
    ...
    @Transactional(readOnly = true)
    @Query("SELECT DISTINCT p FROM ProfileEntity p LEFT JOIN p.stacks s WHERE (s LIKE %:keyword% OR p.nickname LIKE %:keyword%) AND (p.profileType = :profileType OR p.profileType = 'BOTH')")

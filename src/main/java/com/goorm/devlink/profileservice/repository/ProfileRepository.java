...
public interface ProfileRepository extends JpaRepository<ProfileEntity, Long> {//, ProfileRepositoryCustom {
    ...
    @Transactional(readOnly = true)
    @Query("SELECT DISTINCT p FROM ProfileEntity p JOIN p.stacks s WHERE ((s LIKE %:keyword%) OR (p.nickname LIKE %:keword%)) AND (p.profileType = :profileType OR p.profileType = 'BOTH')")

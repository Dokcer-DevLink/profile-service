pipeline {
    ...
    environment {
        ...
        IMAGE_NAME = 'profile-service' // Your DockerHub repository name
...
stage('K8S Manifest Update') {
            steps {
                ...
                dir('gitOpsRepo') {
                    ...
                    sh "sed -i 's/profile-service:.*\$/profile-service:${currentBuild.number}/' ./eks/profile-service.yaml"
                    sh "git add ."
                    sh "git commit -m '[UPDATE] Profile-Service K8S ${currentBuild.number} image versioning'"
